...
import { FlaskConical, CheckCircle2, XCircle, User, ShieldAlert, ArrowRight } from "lucide-react";
import { db } from "../lib/db";
import { assessBloodExchange } from "../lib/safetyAssessment";
...
export default function Compatibility() {
  ...
  const handleCheck = (e) => {
    ...
    const donorReports = db.bloodTestReports.filter((r) => r.donorProfileId === donor.id);
    setAssessment(assessBloodExchange({ donor, recipient, donorReports }));
  };
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleCheck} className="bg-white rounded-2xl border border-crimson-100 p-6 mt-6 space-y-5">
        ...
        >
          <FlaskConical size={16} /> Check Compatibility
        </button>
      </form>
            ...
            {assessment.needsCaution && (
              ...
              >
                ...
                <ul className="mt-2 text-sm text-amber-800/90 list-disc list-inside">
                  <li>No approved test reports available. Proceed only with medical supervision.</li>
                </ul>
              </motion.div>
            )}
