...
import { db, reserveBloodUnit, releaseReservation, releaseExpiredReservations } from "../lib/db";
...
export default function BloodBanks() {
  ...
  const filtered = useMemo(
    ...
    [banks, query] // eslint-disable-line react-hooks/exhaustive-deps
  );

  const handleReserve = (bank, group) => {
    const result = reserveBloodUnit({
      bankId: bank.id,
      bloodGroup: group,
      userId: user.id,
  ...
  const handleRelease = (reservationId) => {
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-crimson-950 truncate">{r.bankName}</p>
                      <p className="text-xs text-crimson-900/50">{timeLeftLabel(r.expiresAt)}</p>
                    </div>
                  </div>
                  <button
