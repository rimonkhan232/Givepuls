...
export default function AdminRequests() {
  ...
  const markFulfilled = (r) => {
    db.bloodRequests.update(r.id, { status: r.status === "fulfilled" ? "open" : "fulfilled" });
    setRequests(db.bloodRequests.list());
  };

  const removeRequest = (r) => {
    if (!window.confirm("Delete this blood request?")) return;
    db.bloodRequests.remove(r.id);
    setRequests(db.bloodRequests.list());
  };
