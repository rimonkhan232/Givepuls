...
export default function Home() {
  ...
  const donors = useMemo(() => db.donorProfiles.list(), []);
  const banks = db.bloodBanks.list();
  const activeDonorCount = useMemo(() => donors.filter((d) => d.available).length, [donors]);
  const livesTouched = useMemo(
    () => db.donations.list().length + db.bloodRequests.filter((r) => r.status === "fulfilled").length,
    []
  );
  const districtsCovered = useMemo(() => {
    const set = new Set([...banks.map((b) => b.division), ...donors.map((d) => d.division)]);
    return set.size;
  }, [banks, donors]);
  const openRequests = useMemo(
    () => db.bloodRequests.filter((r) => r.status === "open"),
    []
  );
  const myProfile = useMemo(
    () => (user ? donors.find((p) => p.userId === user.id) : null),
    [donors, user]
  );
  const [requestTab, setRequestTab] = useState(myProfile?.division ? "nearby" : "all");
  const nearbyRequests = useMemo(
    () =>
      myProfile?.division
        ? openRequests.filter((r) =>
            r.location?.toLowerCase().includes(myProfile.division.toLowerCase())
          )
        : [],
    [openRequests, myProfile?.division]
  );
  const requests = (requestTab === "nearby" ? nearbyRequests : openRequests).slice(0, 3);
      ...
      <section className="relative gradient-brand pb-28 pt-16 sm:pt-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              <div className="glass rounded-3xl p-6 shadow-2xl shadow-black/20 border border-white/40">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <button
                    ...
                    disabled={!myProfile?.division}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${
                      requestTab === "nearby"
                        ? "bg-crimson-700 text-white"
                        : "bg-white/70 text-crimson-800 hover:bg-white"
                    }`}
                  >
                    {t("nearMe")}{myProfile?.division ? ` (${myProfile.division})` : ""}
                  </button>
                  <button
                    type="button"
                ...
                <div className="space-y-3">
                  {requests.length === 0 && (
                    <p className="text-sm text-crimson-900/50 py-8 text-center">
                      {requestTab === "nearby" ? t("noRequestsNearby") : "No open requests right now."}
                    </p>
                  )}
                  {requests.map((r) => (
                    <div
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-20 pt-10 border-t border-white/10">
            <Stat value={livesTouched} label={t("livesTouched")} />
            <Stat value={activeDonorCount} label={t("activeDonors")} />
            <Stat value={banks.length} label={t("bloodBanks")} />
            <Stat value={districtsCovered} label={t("citiesCovered")} />
          </div>
        </div>
      </section>
